<template>
  <div>
    <div class="card mb-10">
      ...
      <div class="card-name">
        用户 <span class="text-gray">{{ list.length }}</span>
      </div>
      <n-spin :show="loading">
        <n-scrollbar style="height: 335px">
          <div
            class="user-item flex align-center justify-between"
            v-for="item in list"
            @click="edit(item)"
          >
            <div>
              ...
              <n-tag size="small" :bordered="!item.admin" type="success" class="mr-5"
                >管理权限</n-tag
              >
              <n-tag size="small" :bordered="!item.edit" type="success">编辑权限</n-tag>
            </div>
            <n-button
              @click.stop="remove(item.id, item.nickname)"
              strong
              secondary
              circle
              type="error"
...
export default {
  ...
  methods: {
    ...
    submitAdd(form) {
      user
        .add(form.name, form.pass, form.nickname, form.admin, form.edit)
        .then((res) => {
    ...
    submitEdit(form) {
      user
        .edit(form.id, form.name, form.pass, form.nickname, form.admin, form.edit)
        .then((res) => {
          this.form.loading = false;
          if (res.state) {

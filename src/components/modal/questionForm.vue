    ...
    <n-form ref="form" :disabled="form.loading" :model="form" :rules="rules">
      ...
      <div class="flex align-center">
        ...
        <n-form-item
          ...
          path="type"
          label="题型"
          style="min-width: 90px; width: 160px"
        >
          <n-select
      ...
      <template v-if="form.type == 1 || form.type == 2">
        ...
        <div class="form-ops mb-10">
          <div
            ...
            class="form-ops-item flex align-center mt-10"
            v-for="(_item, index) in form.options"
          >
            <n-text class="form-ops-index text-center mr-5" type="success">{{
              index + 1
            }}</n-text>
            <n-input
              v-model:value="form.options[index].label"
              :disabled="form.loading"
              type="text"
              placeholder="请输入选项内容"
            />
          ...
          <div v-else class="text-center">
            <n-text depth="3">请点击右侧加号添加选项</n-text>
          </div>
        </div>
      ...
      <template v-if="form.type == 4">
        ...
        <div class="form-ops mb-10">
          ...
          >
            ...
            <n-input
              v-model:value="form.answer[index]"
              :disabled="form.loading"
              type="text"
          ...
          <div v-else class="text-center">
            <n-text depth="3">请点击右侧加号添加每空答案</n-text>
          </div>
        </div>
      </template>
      <!-- 对号题 -->
...
export default {
  ...
  methods: {
    ...
    checkForm(form) {
      ...
      if (form.options != null && form.options instanceof Array)
    ...
    submitAdd(form) {
      question
        .add(form.sid, form.level, form.type, form.question, form.options, form.answer)
        .then((res) => {

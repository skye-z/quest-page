<template>
    <n-modal v-model:show="form.show" preset="card" :style="{ width: '800px' }" :mask-closable="false">
        <template #header>
            {{ form.id ? '编辑试卷' : '添加试卷' }}
        </template>
        <div class="flex justify-between">
            <n-form ref="form" :disabled="form.loading" :model="form" :rules="rules" style="width: 400px;">
                <n-form-item path="name" label="考试名称">
                    <n-auto-complete v-model:value="form.name" clear-after-select placeholder="请输入考试名称" />
                </n-form-item>
                <div class="flex">
                    <n-form-item class="full-width mr-10" path="sid" label="科目">
                        <n-select v-model:value="form.sid" @update:value="getQuestionNumber" placeholder="请选择科目"
                            :options="subjects" />
                    </n-form-item>
                    <n-form-item v-if="form.sid" class="full-width" label="关键词">
                        <n-auto-complete v-model:value="form.keyword" clear-after-select placeholder="请输入题目关键词" />
                    </n-form-item>
                </div>
                <n-scrollbar style="height: 400px;border-radius: 12px;margin-top: -10px;" trigger="none">
                    <div v-for="item in list" class="question-item pa-5 border-bottom" @click="addQuestion(item)">
                        {{ item.question }}
                    </div>
                </n-scrollbar>
                <n-pagination v-if="number > 0" class="justify-center mt-10" :page-slot="7" v-model:page="page"
                    :item-count="number" @update:page="getQuestionList" />
            </n-form>
            <div class="full-width ml-10" v-if="form.sid">
                <div class="mb-10" style="line-height: 14px;">卷面 <span class="text-gray">{{ form.questions.length }}</span>
                </div>
                <n-scrollbar style="max-height: 535px;border-radius: 12px;" trigger="none">
                    <div v-for="(item, index) in form.questions" class="question-item pa-5 border-bottom"
...
export default {
    ...
    methods: {
        ...
        getQuestionNumber() {
            ...
            this.number = 0
            this.list = []
            question.getNumber(this.form.sid, null).then(res => {
                this.number = res.num
                if (res.num > 0) {
                    this.page = 1
        ...
        getQuestionList() {
            question.getList(this.form.sid, null, this.page, 10).then(res => {
                if (res.list) {
                    this.list = res.list
                } else {
        ...
        submitAdd(form) {
            exam.add(form.name, form.sid, form.questions).then(res => {
        ...
        submitEdit(form) {
            exam.edit(form.id, form.name, form.sid, form.questions).then(res => {
                this.form.loading = false
                if (res.state) {

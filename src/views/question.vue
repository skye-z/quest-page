      ...
      <div v-else>
        <div class="question-item card mb-10" v-for="item in questions">
          ...
          <div class="question-title">
            ...
            <n-text v-else :type="checkState[item.id] == true ? 'success' : 'error'">{{
              item.question
            }}</n-text>
          </div>
          <template v-if="item.type == 1">
            <n-radio-group
              @update:value="(value) => check(item, value)"
              class="mt-10"
          ...
          <template v-else-if="item.type == 4">
            <div class="question-ops">
              <n-input
                ...
                @blur="check(item)"
                v-model:value="form[item.id][index]"
...
export default {
  ...
  methods: {
    ...
    getQuestionList() {
      ...
      this.questions = [];
      question
        .getList(this.subject, null, this.page, 20)
        .then((res) => {
